...
if (isset($_POST['student_id'], $_POST['academic_year'], $_POST['term'], $_POST['exam'], $_POST['sub_id'], $_POST['form'], $_POST['subname'])) {
    ...
    try {
        ...
        header("Location: major3.php?year={$academicYear}&term={$term}&sub={$subID}&form={$_POST['form']}&exam={$_POST['exam']}&success=" . urlencode($successMessage));
        exit();
        ...
        header("Location: major3.php?year={$academicYear}&term={$term}&sub={$subID}&form={$_POST['form']}&exam={$_POST['exam']}&error=" . urlencode("Error updating scores: " . $e->getMessage()));
        exit();
    }
} else {
    // Redirect back to major4.php with error message if parameters are missing
    header("Location: major3.php?year={$_POST['academic_year']}&term={$_POST['term']}&sub={$_POST['sub_id']}&form={$_POST['form']}&exam={$_POST['exam']}&error=" . urlencode("Missing parameters"));
    exit();
}
?>
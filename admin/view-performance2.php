...
include('includes/dbconnection.php');
    ...
    if (isset($_GET['id']) && isset($_GET['name']) && isset($_GET['class']) && isset($_GET['gen'])) {
        ...
        $sqlApprove = "UPDATE tblstudent SET Approve_status = 'Yes' WHERE ID = :stuID";
        $sqlApprove = $dbh->prepare($sqlApprove);
        $sqlApprove->bindParam(':stuID', $stuID, PDO::PARAM_INT);
        $sqlApprove->execute();
...
.bank-section p {
    margin: 0; /* Remove default margin */
    font-size: 12px; /* Set font size for details */
}
...
<button type="button" class="btn" onclick="generatePDF()" 
    ...
    <i class="fas fa-download" style="font-size: 20px;"></i>
                         ...
                         <div class="card-body">
    <a href="previous-results.php?user=<?php echo $user; ?>&id=<?php echo $stuID; ?>" 
       class="btn" 
       style="display: block; margin: 0 auto; background-color: #003366; color: white; border-radius: 40px; padding: 10px 20px; text-align: center;">
...
if ($result) {
    ...
    if ($examview == 'not viewable') {
        // Instead of redirecting, display error message directly
        ?>
        <div class="alert alert-danger alert-dismissible fade show" role="alert" style="max-width: 800px; margin: 20px auto; padding: 20px; border-radius: 5px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
            <h4 class="alert-heading"><i class="fas fa-exclamation-triangle"></i> Results Not Available (<?php echo htmlspecialchars($name); ?>, Form : <?php echo htmlspecialchars($class); ?>)</h4>
            <p>Current academic year (<?php echo htmlspecialchars($activeYear); ?>) and term (<?php echo htmlspecialchars($activeTerm); ?>) results are not currently viewable.</p>
            <hr>
            <p class="mb-0">You can view your previous results by clicking the button below.</p>
            <div class="mt-3">
                <a href="previous-results.php?user=unknown&id=<?php echo urlencode($stuID); ?>" class="btn" style="background:#003366; color:white;">
                    <i class="fas fa-archive"></i> View Previous Results
                </a>
            </div>
                    </div>
        <?php
        // You might want to include additional styling or exit the current page rendering
...
if ($queryEnglishGrade && $queryEnglishGrade->rowCount() > 0) {
    ...
    if ($englishGrade == 9 || $englishGrade == "") {
        $result = 'FAILED';
    } else {
...
if ($queryEnglishGrade && $queryEnglishGrade->rowCount() > 0) {
    ...
    // Check if the English grade is 9
    if ($englishGrade == "F" || $englishGrade == "") {
        $result = 'FAILED';
    } else {
        // Check if the total points fall within the passing range
        $result = ($totalPoints >= 240 && $totalPoints < 601) ? 'PASSED' : 'FAILED';
    }
} else {
    // English grade is not available or query unsuccessful
...
// SQL query to count the number of subjects where final_grade is 40 or more
$sqlpasscount = "
    ...
    WHERE stuID = :stuID 
        AND academic_year = :academicYear 
        AND term = :term 
        AND final_grade >= 40 -- Include only grades >= 40
        AND final_grade REGEXP '^[0-9]+$' -- Exclude non-numeric values
";

// Prepare the query
$stmt = $dbh->prepare($sqlpasscount);
...
$passCount = $result['pass_count']; // Number of subjects with final_grade >= 40

// Determine pass/fail based on the count
$result = ($passCount >= 6) ? 'PASSED' : 'FAILED';
...
// Bind the parameters
$queryUpdate->bindParam(':result', $result, PDO::PARAM_STR);
$queryUpdate->bindParam(':stuID', $stuID, PDO::PARAM_INT);
$queryUpdate->bindParam(':class', $class, PDO::PARAM_STR);
$queryUpdate->bindParam(':term', $term, PDO::PARAM_STR);
$queryUpdate->bindParam(':academicYear', $academicYear, PDO::PARAM_STR);
...
?></i></p>
    ...
    <div class="text-badge" style="display: inline; text-align: justify; font-size: 1px; line-height: 1.0;">
      <?php  $remarks = [
    'FAILED' => 'You can do better! Keep working hard and focus on improving your weaknesses.',
    'PASSED' => 'Great performance! Continue working hard and striving for excellence.',
];
?>
<p>
    <b>HEAD TEACHER'S REMARK: </b> 
    <i>
        <?php echo isset($remarks[$result]) ? '. ' . $remarks[$result] : ''; ?>
    </i>
</p>
    </div>
    <div class="text-badge"style="display: inline; text-align: justify; font-size: 1px; line-height: 1.0;">

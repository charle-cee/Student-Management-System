        ...
        if (isset($_POST['submit'])) {
            ...
            if ($result) {
                ...
                if ($query->execute()) {
                    ...
                    header('location: staff-borrow-action.php?id=' . urlencode($staffID) . '&name=' . urlencode($name) . '&role=' . urlencode($role) . '&success=' . urlencode($successMessage));
                    exit();
                } else {
                    // Redirect with error message
                    header('location: staff-borrow-action.php?id=' . urlencode($staffID) . '&name=' . urlencode($name) . '&role=' . urlencode($role) . '&error=Failed to borrow book.');
                    exit();
                }
                
            } else {
                header('location: staff-borrow-action.php?id=' . urlencode($staffID) . '&name=' . urlencode($name) . '&role=' . urlencode($role) . '&error=The selected book may have been borrowed by someone.');
                exit();
            }
        }
    } else {
        header('location: staff-borrow-action.php?id=' . urlencode($staffID) . '&name=' . urlencode($name) . '&role=' . urlencode($role) . '&error=Invalid book number or ID entered.');
        exit();
    }
}
?>
        ...
        <div class="main-panel">
            <div class="content-wrapper">
                <div class="page-header">
                    <h3 class="page-title"> Staff Borrowing Book </h3>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="dashboard.php">Dashboard</a></li>
                            <li class="breadcrumb-item active" aria-current="page"> Staff Borrowing Book</li>
                        </ol>
                    </nav>
                </div>
                <div class="row">
                    <div class="col-12 grid-margin stretch-card">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title" style="text-align: center; background-color: #003366; color: white; padding: 10px;">Staff Borrow Book</h4>
                                <form class="forms-sample" method="post" enctype="form-data">
                                    ...
                                    <div class="form-group">
                                        <label for="fees">Role</label>
                                        <input type="text" class="form-control" id="className" name="className" readonly value="<?php echo htmlspecialchars($role); ?>">
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <!-- Borrow Button -->
<button type="submit" class="btn mr-2" name="submit" style="background-color: #003366; color: white; padding: 10px; border-radius: 40px; border: none;">
    Borrow
</button>

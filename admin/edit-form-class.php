...
if (strlen($_SESSION['sturecmsaid']==0)) {
    ...
    if (isset($_GET['id'])) {
        $teacherID = $_GET['id'];
        $sqlCheckAllocation = "SELECT * FROM tbladmin WHERE ID = :teacherID";
        ...
        $row = $queryCheckAllocation->fetch(PDO::FETCH_ASSOC);
        if ($row) {
            $name = $row['Name'];
        $class = $row['FormClass'];} else {
            header('location: edit-form-class.php?error=Failed to fetch the teacher details');
            exit();
        }
    ...
    if (isset($_POST['submit'])) {
        ...
        $class = $_POST['class'];
            ...
            if ($query) {
                header('location: edit-form-class.php?id='.$teacherID.'&success= You have re-assigned '.$name.' as a form teacher for '.$class.'.');
                exit();
            } else {
                header('location: edit-form-class.php?error=Failed to Reassign Class.');
                exit();
            }
        }
...
<meta charset="UTF-8">
    ...
    <title>Student Management System|| Reassign Class</title>
...
<div class="container-scroller">
        ...
        <div class="main-panel">
            <div class="content-wrapper">
                <div class="page-header">
                    <h3 class="page-title"> Reassign Class </h3>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="dashboard.php">Dashboard</a></li>
                            <li class="breadcrumb-item active" aria-current="page"> Reassign Class</li>
                        </ol>
                    </nav>
                </div>
                <div class="row">
                    <div class="col-12 grid-margin stretch-card">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title" style="text-align: center; background:#003366; color : white; padding : 10px;">Reassign Class</h4>
    ...
    <div class="input-group">
        ...
        <input type="hidden" class="form-control" value="<?php echo $teacherID; ?>" readonly>
    </div>
</div>
...
<div class="form-group">
    ...
    <select name="class" class="form-control" required>
        ...
        <?php foreach ($subjects as $subject) { ?>
            <option value="<?php echo $subject['ClassName'] . ' ' . $subject['Section']; ?>"><?php echo $subject['ClassName'] . ' ' . $subject['Section']; ?></option>
        <?php } ?>
    </select>
</div>
                                   <!-- Assign Class Button -->
